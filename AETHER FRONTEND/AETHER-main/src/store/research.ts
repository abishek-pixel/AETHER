...
interface ResearchStore {
  ...
  resetCurrent: () => void;
  resetError: () => void;
  /** Clear all in-memory guest sessions (sessions without a DB-backed UUID).
   *  Called on login so guest research never pollutes an authenticated user's history. */
...
export const useResearchStore = create<ResearchStore>((set, get) => ({
  ...
  loadSessionsFromDB: async () => {
    ...
    try {
      ...
      // REPLACE the session list with the authoritative server response.
      // Do NOT merge with in-memory guest sessions — authenticated users must
      // only see their own persisted sessions, never a prior guest's work.
      const fromDB = (data.sessions ?? []).map(dbSessionToLocal);

      // Preserve any in-memory session that is currently streaming (status
      // "idle" or "running") so an in-progress research isn't wiped mid-flight.
      set((state) => {
        const streaming = state.sessions.filter(
          (s) => s.status === "idle" || s.status === "running",
        );
        // Merge: streaming sessions take priority; DB sessions fill the rest.
        const dbIds = new Set(fromDB.map((s) => s.id));
        const stillStreaming = streaming.filter((s) => !dbIds.has(s.id));
        return {
          sessions: [...stillStreaming, ...fromDB],
          isFetchingHistory: false,
        };
      });
    } catch (err) {
  ...
  clearGuestSessions: () => {
    // A "guest" session is one that was never promoted to a DB UUID.
    // DB UUIDs are standard UUID v4 strings (xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx).
    // Local IDs generated by crypto.randomUUID() are also UUID v4, so we can't
    // distinguish purely by format — instead we keep only sessions that were
    // persisted (i.e. status === "done" AND id looks like a UUID, meaning the
    // promoteSessionId path ran) AND exclude any that are currently streaming.
    //
    // The simplest safe rule: drop ALL sessions from in-memory store on login.
    // loadSessionsFromDB() will immediately re-populate with the real user data.
    set((s) => {
      // Keep only sessions actively streaming so we don't cut off an in-progress request.
      const keepStreaming = s.sessions.filter(
        (sess) => sess.status === "idle" || sess.status === "running",
      );
      const cur = s.current;
      const keepCurrent =
        cur && (cur.status === "idle" || cur.status === "running") ? cur : null;
      return { sessions: keepStreaming, current: keepCurrent, error: null };
    });
  },
